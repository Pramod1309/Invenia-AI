...
class Database {
  ...
  // User operations
  async createUser(userData: { name: string; email: string; password: string; googleId?: string; avatar?: string }) {
    const sql = `
  ...
  async getJobsByStatus(status: string, userId?: number) {
  ...
  async incrementJobViews(id: number) {
